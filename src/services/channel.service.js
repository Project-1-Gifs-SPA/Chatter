...
export const addChannel = (teamId, channelName, isPublic, owner, creator, members) => {
  ...
  isPublic
    ? getAllTeamMembers(teamId)
      .then(teamMembers => teamMembers.map(teamMember => (formattedMembers[teamMember] = true)))
...
export const getGeneralChannel = (teamId) =>
  getChannelInTeamByName(teamId, 'General')
...
export const setChannelUsers = (channelId, users) =>
  set(ref(db, `channels/${channelId}/members`),
    {
      ...users,
    }
  );

export const addChannelUser = (channelId, user) =>
  getChannelById(channelId)
    .then(channel => setChannelUsers(channelId, { ...channel.members, user }))
    .catch(e => console.error(e));

export const removeChannelUser = (channelId, user) =>
  getChannelById(channelId)
    .then(channel => {
      const fixedusers = {};
...
export const getAllChannelsByTeam = (teamId) =>
  get(ref(db, `teams/${teamId}/channels`))
    .then(snapshot => Object.keys(snapshot.exists() ? snapshot.val() : {}))
    .catch(e => console.error(e));

export const getAllPublicChannelsByTeam = (teamId) =>
...
export const getAllPrivateChannelsByTeam = (teamId) =>
  getAllChannelsByTeam(teamId)
...
export const getChannelInTeamByName = (teamId, channelName) =>
  getAllChannelsByTeam(teamId)
    .then(channelIds => Promise.all(channelIds.map(channelId =>
      getChannelById(channelId)))
      .then(channels => {
...
export const getChannelIdsInTeamByUser = (teamId, userHandle) =>
  getAllChannelsByTeam(teamId)
    .then(channelIds => Promise.all(channelIds.map(channelId => getChannelById(channelId)))
...
export const editChannelMessage = (content, channelId, msgId) =>
  update(ref(db, `channels/${channelId}/msgs/${msgId}`),
    {
      body: content
    }
  );
...
export const filterChannelsByUser = (channels, userHandle) =>
  Promise.all(channels.map(channelId => getChannelById(channelId)))
    .then(channels => channels.filter(channel => Object.keys(channel.members).includes(userHandle) ? channel : false))
    .catch(e => console.error(e));
